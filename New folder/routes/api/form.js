...
router.post(
  ...
  [
    ...
    check('consumerCode','auth_token is required').not().isEmpty(),
  ],
  async (req, res) => {
    ...
    const { name,fatherName,mobile,email,applicantType,district,areaType,address1,address2,pincode,road_urban_local_body_name,road_ward_no,road_locality,road_cuttingReason,road_category,road_totalcost,auth_token,consumerCode} = req.body;
